...
const connectToDatabase = async () => {
    const client = await MongoClient.connect('mongodb://127.0.0.1:2701'); //Connect to the server(!) .Call the 'connect' static method on it, so we don't need to instantiate MongoClient. Connect returns a promise. What we'll get back is a client object which has internal information about the established connection.
